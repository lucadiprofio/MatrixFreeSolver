...
public:
  ...
  // Constructor.
  ADRMatrixBased3D(const std::string  &mesh_file_name_,
                    const unsigned int &r_,
                    const std::function<double(const Point<dim> &)> &mu_,
                    const std::function<Tensor<1,dim>(const Point<dim> &)> &beta_,
...
protected:
  ...
  // Advection velocity beta(x)
  std::function<Tensor<1,dim>(const Point<dim> &)> beta;
