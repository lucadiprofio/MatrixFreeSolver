...
namespace Project {
  ...
  void SolverClass::init_mesh() {
    ...
    // Se vuoi raggruppare tutti i muri sotto un unico ID (es. 2) per comodità:
  ...
  void SolverClass::setup_system() {
    ...
    // TODO entrambi locally owned e relvent come attributi?
    locally_owned_dofs = dof_handler.locally_owned_dofs();
    DoFTools::extract_locally_relevant_dofs(dof_handler, locally_relevant_dofs);

    constraints.clear();
    //constraints.reinit(locally_owned_dofs, locally_relevant_dofs); TODO update dealii
    ...
    dirichlet_map[4] = &dirichlet_function_walls;
    ...
    additional_data.tasks_parallel_scheme = MatrixFree<dim, double>::AdditionalData::none; // TODO ?
    additional_data.mapping_update_flags = (update_gradients | update_JxW_values | update_quadrature_points); // TODO Li cambio?
  ...
  void SolverClass::setup_multigrid() {
    ...
    for (unsigned int level = 0; level < nlevels; ++level) {
      ...
      additional_data.tasks_parallel_scheme = MatrixFree<dim, float>::AdditionalData::none; // TODO also here
  ...
  void SolverClass::assemble_rhs() {
    ...
    for (unsigned int cell = 0; cell < system_matrix.get_matrix_free()->n_cell_batches(); ++cell) {
      ...
      for (unsigned int q = 0; q < phi.n_q_points; ++q) {
        ...
        phi.submit_value(f_val,q); // Rhs standard
        phi.submit_gradient((tau * f_val) * beta_val,q); // SUPG
      }
  ...
  void SolverClass::solve() {
    TimerOutput::Scope t(computing_timer, "Solve");

    SolverControl solver_control(1000, 1e-12 + 1e-8 * system_rhs.l2_norm());
    MatrixFreeActiveVector distributed_solution(locally_owned_dofs, mpi_communicator);
    ...
    for (unsigned int level = 0; level < triangulation.n_global_levels(); ++level) {
      smoother_data[level].smoothing_range = 20.0; // Alpha 
      smoother_data[level].degree = 2;  // Sweeps

      // 3. Eigen Analysis Iterations (Corrisponde a "eigen-analysis: iterations")
      // Serve per stimare accuratamente l'autovalore massimo per settare i coefficienti.
  ...
  void SolverClass::output_results(const unsigned int cycle) {
  ...
  void SolverClass::estimate_error(const unsigned int cycle) {
    ...
    QIterated<dim> q_iterated(q_trapez, fe.degree*2 + 1); // Constructor takes a one-dimensional quadrature rule and a number that tells it how often it shall repeat this rule in each space direction
  ...
  void SolverClass::track_memory() const {
