...
class ADR_Operator : public MatrixFreeOperators::Base<
                         dim, LinearAlgebra::distributed::Vector<number>> {
public:
  ADR_Operator() = default;

  void clear() override;
  void setup_coefficients(const DiffusionCoefficient<dim> &mu_function,
...
template <int dim, int fe_degree, typename number>
void ADR_Operator<dim, fe_degree, number>::local_apply(
  ...
  for (unsigned int cell = range.first; cell < range.second; ++cell) {
    ...
    for (unsigned int q = 0; q < phi.n_q_points; ++q) {
      ...
      phi.submit_gradient(mu_val * grad_u                        // diffusion
                              - u * beta_val +                   // advection
                              (tau * strong_residual) * beta_val // SUPG
                          ,
                          q);
      phi.submit_value(gamma_val * u // reaction
                       ,
                       q);
    }
...
template <int dim, int fe_degree, typename number>
void ADR_Operator<dim, fe_degree, number>::apply_add(
    LinearAlgebra::distributed::Vector<number> &dst,
...
void ADR_Operator<dim, fe_degree, number>::local_compute_diagonal(
  ...
  for (unsigned int q = 0; q < phi.n_q_points; ++q) {
    ...
    phi.submit_gradient(mu_val * grad_u + (tau * strong_residual) * beta_val,
                        q);
...
void ADR_Operator<dim, fe_degree, number>::compute_diagonal() {
  ...
  LinearAlgebra::distributed::Vector<number> &inv_diag =
      this->inverse_diagonal_entries->get_vector();
  this->data->initialize_dof_vector(inv_diag);

  MatrixFreeTools::compute_diagonal(
      *this->data, inv_diag, &ADR_Operator::local_compute_diagonal, this);
