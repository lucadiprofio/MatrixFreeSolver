...
namespace MatrixBased {
  ...
  void SolverClass::setup_system() {
    ...
    // VectorTools::interpolate_boundary_values(mapping, dof_handler, 4, dirichlet_function_inlet, constraints);
    constraints.close();
  ...
  void SolverClass::assemble_system() {
    ...
    std::vector<Vector<double>> b_values(n_q_points, Vector<double>(dim));
    ...
    for (const auto &cell : dof_handler.active_cell_iterators()) {
      ...
      b_function.vector_value_list(fe_values.get_quadrature_points(), b_values);
 
      for (unsigned int q = 0; q < n_q_points; ++q) {

        for (unsigned int d = 0; d < dim; ++d) b_loc[d] = b_values[q][d];
  
        // SUPG parameter
        const double h = cell->diameter();
        const double vel_norm = b_loc.norm();

        double tau_inv_sq =
          (2.0 * vel_norm / h) * (2.0 * vel_norm / h)
          + 9.0 * (4.0 * mu_values[q] / (h * h)) * (4.0 * mu_values[q] / (h * h))
          + (sigma_values[q]) * (sigma_values[q]);

        // double tau = 1.0 / std::sqrt(tau_inv_sq);
        double tau = 0.0 / std::sqrt(tau_inv_sq); // SUPG off
        if (tau_inv_sq < 1e-12) tau = 0.0;
      
        for (unsigned int i = 0; i < dofs_per_cell; ++i) {
          const double phi_i = fe_values.shape_value(i, q);
          const Tensor<1, dim> grad_phi_i = fe_values.shape_grad(i, q);

          const double supg_test_i = tau * (b_loc * grad_phi_i);
          ...
          for (unsigned int j = 0; j < dofs_per_cell; ++j) {
            ...
            cell_matrix(i, j) += (
              ...
              + ((b_loc * grad_phi_j) + sigma_values[q] * phi_j) * supg_test_i // SUPG
            ) * fe_values.JxW(q);
        ...
        for (const auto &face : cell->face_iterators())
          if (face->at_boundary()) {
            ...
            if (neumann_boundary_ids.count(id)) {
              fe_face_values.reinit(cell, face); // Set on that face
  ...
  /*  Vector<float> level(triangulation.n_active_cells());
    ...
    data_out.build_patches(); // TODO is mapping Q1 the best choice?
